...
.controller('loginCtrl', function($scope, $cordovaBarcodeScanner, $state, $http, $ionicPopup, $translate, $ionicLoading, $filter) {
    ...
    var master = {devel: false, port: 80, style: {'background-color':'#CECECE'}};
    var devel = {devel: true, port: 8080, style: {'background-color':'#00FF00'}};
    $scope.server = master;
    ...
    $scope.login = function() {
        ...
        /** Data for server */
        var url = 'http://147.228.63.49:' + $scope.server.port + '/app/mobile-services/login';
...
.controller('listOfMeasurementsCtrl', function($scope, $state, $http, $translate, $ionicLoading, $filter) {
    ...
    $scope.$on('$ionicView.beforeEnter', function(){   
        ...
        /** Show loading */
         ...
         /** Data for server */
         var url = 'http://147.228.63.49:' + $scope.server.port + '/app/mobile-services/measurement-list';
...
.controller('measurementCtrl', function($scope, $state, $cordovaBarcodeScanner, $ionicPopup, $http, $translate, $ionicLoading, $filter) {
    ...
    $scope.$on('$ionicView.beforeEnter', function(){
        ...
        /** Data for server */
        var url = 'http://147.228.63.49:' + $scope.server.port + '/app/mobile-services/scheme';
        var data = {'client_username': $scope.username, 
                    'token': $scope.token, 
...
.controller('resultsCtrl', function($scope, $state, $cordovaBarcodeScanner, $ionicPopup, $http, $translate, $ionicLoading, $filter) {
    ...
    $scope.showConfirm = function() {
        ...
        /** Data for server */
        var url = 'http://147.228.63.49:' + $scope.server.port + '/app/mobile-services/measured-data';
        var data = {'client_username':$scope.username,
                    'token':$scope.token,
                    'personID':$scope.person.personID                    
                    };
...
.controller('menuCtrl', function ($scope, $state, $ionicPopup, $http, $translate, $ionicLoading, $filter) {
    ...
    $scope.logout = function(){
        ...
        /** Data for server */
        var url = 'http://147.228.63.49:' + $scope.server.port + '/app/mobile-services/logout';
        var data = {'client_username': $scope.username, 
                    'token': $scope.token, 
                   };
